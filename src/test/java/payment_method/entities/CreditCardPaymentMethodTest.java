...
public class CreditCardPaymentMethodTest {
    ...
    private static final PaymentProvider PROVIDER = new PaymentProvider(
            "provider-1", "Visa", PaymentProviderSupportStatus.SUPPORTED
    );
    private static final String MASKED_IDENTIFIER = "****1234";

    // Verify every required credit-card field rejects null with the exact message.
    @ParameterizedTest(name = "Reject null {0}")
    @MethodSource("nullArguments")
    @DisplayName("Throw NullPointerException for null credit-card data")
    ...
    @DisplayName("Throw IllegalArgumentException for blank credit-card data")
